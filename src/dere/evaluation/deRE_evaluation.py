...
def read_a1_file(filename: str) -> Tuple[Dict[str, Tuple[str, int, int]], Dict[int, str]]:
    events_in_text = {}
    span_annotations = {}
    with open(filename) as f:
        for line in f:
...
def read_a2_file(
        ...
        mode: str
                ...
                Dict[str, int], 
                Dict[int, str], 
                Dict[str, str]]: # mode: either G for gold or A for predicted answer
    ...
    if mode == 'A':
            ...
            for r in remain:
                ...
                for item in frame_annotations[r][2]:
                    if re.search(r'\:E[0-9-]+$', item):
                        e_arg.append(item)
                e_aid = [parts[1] for e_arg_item in e_arg for parts in e_arg_item.split(':')]
                danglingp = 0
                for e_a in e_aid:
                    if not e_a in added:
                        danglingp = 1
                ...
                if danglingp == 0:
                    ...
                    changep = 1
            if changep == 0:
                logging.info("circular reference: [" + filename + "]" + ", ".join(remain))
        ...
        for e_id in e_list:
            ...
            if eventexp_key in eventexp:
                ...
                equiv[e_id] = d_id
                logging.info("[" + filename + "]" + e_id + " is equivalent to " + d_id + " => removed.")
            else:
    ...
    return span_annotations, frame_annotations, num_event, num_span, events_in_text, equiv
...
def report_headline() -> None:
    print("Class".ljust(20) + "\t" + "gold (match)".ljust(10) + "\t" + "answer (match)".ljust(10) 
            + "\t" + "recall \t prec. \t fscore")

def report(cl: str, gold: int, matched_gold: int, answer: int, matched_answer: int) -> None:
    ...
    print(cl.ljust(20) + "\t" + gold_column.ljust(10) + "\t" + answer_column.ljust(10) + "\t" 
            + str(round(recall, 2)) + "\t" + str(round(precision, 2)) + "\t" + str(round(f1, 2)))
...
def eq_event(
        ...
        do_soft_args: bool, 
        do_soft_span: bool, 
        do_soft_overlap_span: bool) -> bool:
    if re.search(r'^E', aid):
        if (eq_class(aid, gid, a1_annotations, answers_span, answers_frame, golds_span, golds_frame, 
            do_soft_class)
        and eq_span(aid, gid, a1_annotations, answers_span, answers_frame, golds_span, golds_frame, 
                text, events_in_text, text_len, do_soft_span, do_soft_overlap_span) 
        and eq_args(aid, gid, a1_annotations, answers_span, answers_frame, golds_span, golds_frame, text, 
                events_in_text, text_len, do_soft_class, do_soft_args, do_soft_span, do_soft_overlap_span)):
            return True
...
def eq_args(
    ...
    for i in range(0, len(ae_args)):
        ...
        if aaid[0] != gaid[0]:
            return False
        if (re.search(r'^E', aaid) and not eq_revent(aaid, gaid, a1_annotations, answers_span, answers_frame, 
            golds_span, golds_frame, text, events_in_text, text_len, do_soft_class, do_soft_args, do_soft_span, 
            do_soft_overlap_span)):
            return False
        if (re.search(r'^T', aaid) and not eq_entity(aaid, gaid, a1_annotations, answers_span, answers_frame, 
            golds_span, golds_frame, text, events_in_text, text_len, do_soft_class, do_soft_span, 
            do_soft_overlap_span)):
            return False
    return True
...
def expand_span(
        ...
        text: str, 
        events_in_text: Dict[int, str], 
        text_len: int) -> Tuple[int, int]:
    ebeg = beg - 2
    while ((ebeg >= 0) and (text[ebeg:ebeg+1] not in [" ", ".", "!", "?", ",", "'", '"']) 
            and (ebeg not in events_in_text or events_in_text[ebeg] != "E")):
        ebeg -= 1
    ebeg += 1

    eend = end + 2
    while ((eend <= text_len) and (text[eend-1:eend] not in [" ", ".", "!", "?", ",", "'", '"']) 
...
def eq_revent(
    ...
    if not re.search(r'^E', geid):
        ...
        return False
    if (eq_class(aeid, geid, a1_annotations, answers_span, answers_frame, golds_span, golds_frame, 
        do_soft_class) and eq_span(aeid, geid, a1_annotations, answers_span, answers_frame, golds_span, 
            golds_frame, text, events_in_text, text_len, do_soft_span, do_soft_overlap_span) 
        and eq_args(aeid, geid, a1_annotations, answers_span, answers_frame, golds_span, golds_frame, text, 
            events_in_text, text_len, do_soft_class, do_soft_args, do_soft_span, do_soft_overlap_span)):
        return True
    else:
        return False
...
def count_match(
    ...
    for aid in answer:  # for each answer
        for gid in gold:  # search for golds which match it
            if (eq_event(aid, gid, a1_annotations, answers_span, answers_frame, golds_span, golds_frame, text, 
                events_in_text, text_len, do_soft_class, do_soft_args, do_soft_span, do_soft_overlap_span)):
...
def count_match_span(
    ...
    for aid in answer:  # for each answer
        for gid in gold:  # search for golds which match it
            if (eq_span(aid, gid, a1_annotations, answers, {}, golds, {}, text, events_in_text, text_len, 
                do_soft_span, do_soft_overlap_span)):
                if not aid in cnt_manswer:
                    cnt_manswer[aid] = 0
                if not gid in cnt_matched_gold:
                    cnt_matched_gold[gid] = 0
...
def deRE_evaluation(hypo: str, gold: str, verbose: bool, soft_span: bool, soft_overlap_span: bool) -> None:
    ...
    target_eclass = ["Gene_expression", "Transcription", "Protein_catabolism", "Phosphorylation", 
            "Localization"]
    ...
    for f in files:
        ...
        gold_span_annotations, gold_frame_annotations, num_gold, num_gold_span, events_in_text, equiv \
                = read_a2_file(gold_dir + "/" + pmid + ".a2.t1", events_in_text, equiv, 'G')

        # read answers from system
        answer_span_predictions, answer_frame_predictions, num_answer, num_answer_span, events_in_text, equiv\
                = read_a2_file(f_dir + "/" + f_base, events_in_text, equiv, 'A')

        # get number of matched spans
        num_matched_answer_span, num_matched_gold_span = count_match_span(answer_span_predictions, 
                gold_span_annotations, a1_annotations, text, events_in_text, text_len, do_soft_span, 
                do_soft_overlap_span)
        ...
        num_matched_answer, num_matched_gold = count_match(answer_span_predictions, answer_frame_predictions, 
                gold_span_annotations, gold_frame_annotations, a1_annotations, text, events_in_text, text_len, 
                do_soft_class, do_soft_args, do_soft_span, do_soft_overlap_span)
    ...
    for cl in target_class:  # TODO: get target classes from specification!
        ...
        if cl == "Speculation":
            continue
        report(cl, tnum_gold_span[cl], tnum_matched_gold_span[cl], tnum_answer_span[cl], 
    ...
    report("=[TOTAL]=", tnum_gold_span_value, tnum_matched_gold_span_value, tnum_answer_span_value, 
    ...
    for cl in target_eclass:
        report(cl, tnum_gold[cl], tnum_matched_gold[cl], tnum_answer[cl], tnum_matched_answer[cl])
    ...
    report("=[SVT-TOTAL]=", tnum_gold_value, tnum_matched_gold_value, tnum_answer_value, 
    ...
    report("=[EVT-TOTAL]=", tnum_gold_value, tnum_matched_gold_value, tnum_answer_value, 
            tnum_matched_answer_value)
    print("----------------------------------------------")
    ...
    report("=[REG-TOTAL]=", tnum_gold_value, tnum_matched_gold_value, tnum_answer_value, 
            tnum_matched_answer_value)
    print("----------------------------------------------")
    ...
    report("=[ALL-TOTAL]", gnum_gold_value, gnum_matched_gold_value, gnum_answer_value, 
